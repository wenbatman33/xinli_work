...
export default {
  ...
  mutations: {
    ...
    updateNewsNotify(state, payload) {
      state.newsNotify = true;
    },
    readedNewsNotify(state, payload) {
  ...
  actions: {
    NewsGroup({ commit }, payload) {
      axios.post(`${process.env.VUE_APP_APIHOST}/Web_Page/NewsGroup`, payload)
        .then((res) => {
    ...
    NewsList({ commit }, payload) {
      axios.post(`${process.env.VUE_APP_APIHOST}/Web_Page/NewsList`, payload)
        .then((res) => {
    ...
    CheckLastNewsTime({ commit, state, rootState }, payload) {
      // console.log(window.localStorage['tgNewsTime_' + rootState.member.UserId])
