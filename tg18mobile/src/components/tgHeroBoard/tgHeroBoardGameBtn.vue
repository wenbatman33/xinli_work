...
.bonustotal_cpt
  ...
  .bonustotal_info
    h1 {{gameData.UserName}} 赢了
      span ￥{{gameData.Amount | commaFormat}}
    h2 在
      span {{GameName}}
...
//- .HeroBoardItem
  ...
  .info
    p
      span.channel {{gameData.Username}} 赢了
      span.bonus ￥{{gameData.Amount | commaFormat}}
...
export default {
  ...
  methods: {
    ...
    playGame(type) {
      ...
      // Game maintenance
      if (this.gameData.Status === 2 || this.gameData.Status === '2' || this.gameData.GroupStatus === 0 || this.gameData.GroupStatus === '0') {
        vm.$message({ message: '遊戲維護中', type: 'error' });
      } else {
        ...
        this.$store.dispatch('index/JwtExpireCheck')
          .then((res) => {
            if (res.data.status === 1) {
              ...
              this.addGameRecord();
              this.GA_game_play('排行榜', 'click', this.GAlabel, this.gameData.Sn);
            } else {
              newPage.close();
            }
          });
