...
.membercashback_content
	ol.notes
		li 返水不限平台，最低金额为 1 元。
		li 每天05点後返水紅利將自動更新，请会员当天23:59前申请。
	.membercashback_tab
		select(v-model="selected" @change='handchange')
			option(:value='"all"' @click='filterGroup("all")') 全部
			option(v-for="(item , key) in gameGroup" :value='key' @click='filterGroup(key)') {{item.GroupName}}
		...
		li(v-for="item in mainItem")
			a.membercashback_bonus(@click="goDetail(item)")
				...
				.membermorecoupon_info
					//- 平台 & 返水比例
					h1 {{ item.ChannelName }} 返水 {{ item.OfferRate }} %
					//- 返水金額
				...
				.membercashback_note
					...
					//- 日期
					h2 {{ item.CalcDate }}
	form(v-if="MemberBonus.MemberReturnLog.data_list")
		.memberhistory_btn(v-if="MemberBonus.MemberReturnLog.data_list.length < MemberBonus.MemberReturnLog.total_rows" @click="getMore(filterGroupId)") 显示更多纪录
...
export default {
  ...
  computed: {
    ...
    showMemberBonus() {
      const tempArr = {};
      if (this.MemberBonus.MemberReturnLog.data_list !== undefined) {
        this.MemberBonus.MemberReturnLog.data_list.forEach((item, index, array) => {
  ...
  methods: {
    handchange() {
      this.filterGroup(this.selected);
    ...
    formatStatus(value) {  
      const target = this.MemberBonus.MemberReturnOther;
      let Status = '';
    ...
    getAPI(groupID = false) {
      if (this.UserId) {
        ...
        this.$store.dispatch('member/MemberGetCashLog_return', payload);
      }
    },
  ...
  mounted() {
    if (this.MemberBonus.MemberReturnLog.data_list === undefined || this.MemberReloadSwitch === true) {
      ...
      this.DateRange = [];
      this.DateRange.push(this.MemberBonus.MemberReturnSearchDate.Date_S);
      this.DateRange.push(this.MemberBonus.MemberReturnSearchDate.Date_E);
      this.currentPages = Math.ceil(this.MemberBonus.MemberReturnLog.data_list.length / 30); 
    }
