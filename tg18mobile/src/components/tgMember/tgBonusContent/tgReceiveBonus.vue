<template lang='pug'>
div
  h6.mx-1.my-3.px-2 请选择使用红利的游戏品牌
  .row.no-gutters.px-2
    .col-3.providerItem.my-1.px-1(v-for="(item, index) in applyBonusArray",v-if='btnShowMore(index)',:key='index',v-bind:class='[activeNum === index ? "active" : "" ]', @click='setApplyBrand(item.CompanyName, item.GroupId, item.GroupName, item.GameType, index)')
      .d-flex.flex-column.justify-content-center.align-items-center.w-100.p-1
        .providerImage.mx-2(:style='{background:item.ColorCode}')
          img(:src='item.Images')
        p.providerName.text-center.py-1 {{item.CompanyName}} 
