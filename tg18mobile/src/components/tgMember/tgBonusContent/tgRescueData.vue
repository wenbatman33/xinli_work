		...
		li(v-for="item in mainItem")
			a.membercashback_bonus(@click="goDetail(item)")
				...
				.membermorecoupon_note
					...
					//- 日期
					h2 {{ item.CalcDate }}
	form(v-if="MemberBonus.MemberRescueLog.data_list")
		.memberhistory_btn(v-if="MemberBonus.MemberRescueLog.data_list.length < MemberBonus.MemberRescueLog.total_rows" @click="getMore(filterGroupId)") 显示更多纪录
...
export default {
  ...
  computed: {
    ...
    showMemberBonus() {
      const tempArr = {};
      if (this.MemberBonus.MemberRescueLog.data_list !== undefined) {
        this.MemberBonus.MemberRescueLog.data_list.forEach((item, index, array) => {
  ...
  mounted() {
    if (this.MemberBonus.MemberRescueLog.data_list === undefined || this.MemberReloadSwitch === true) {
      ...
      this.DateRange = [];
      this.DateRange.push(this.MemberBonus.MemberRescueSearchDate.Date_S);
      this.DateRange.push(this.MemberBonus.MemberRescueSearchDate.Date_E);
      this.currentPages = Math.ceil(this.MemberBonus.MemberRescueLog.data_list.length / 30); 
    }
