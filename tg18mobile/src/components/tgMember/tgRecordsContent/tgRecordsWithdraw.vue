...
.container-fluid
	...
	form(v-if="MemberRecords.MemberWithdrawLog.data_list")
		.memberhistory_btn(v-if="MemberRecords.MemberWithdrawLog.data_list.length < MemberRecords.MemberWithdrawLog.total_rows" @click="getMore") 显示更多纪录
...
export default {
  ...
  computed: {
    ...
    showMemberRecords() {
      const tempArr = {};
      if (this.MemberRecords.MemberWithdrawLog.data_list !== undefined) {
        this.MemberRecords.MemberWithdrawLog.data_list.forEach((item, index, array) => {
  ...
  methods: {
    ...
    formatStatus(value) {  
      const target = this.MemberRecords.MemberWithdrawOther.StateCode;
      let Status = '';
    ...
    getAPI() {
      if (this.UserId) {
        ...
        this.$store.dispatch('member/MemberGetCashLog_Withdraw', payload);
      }
    },
    getMore() {
      if (this.UserId) {
  ...
  mounted() {
    if (this.MemberRecords.MemberWithdrawLog.data_list === undefined || this.getSwitch === 1 || this.getSwitch === '1') {
      ...
      this.DateRange = [];
      this.DateRange.push(this.MemberRecords.MemberWithdrawSearchDate.Date_S);
      this.DateRange.push(this.MemberRecords.MemberWithdrawSearchDate.Date_E);
      this.currentPages = this.MemberRecords.MemberWithdrawLog.data_list.length / 30;
    }
