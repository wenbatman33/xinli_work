<template lang="pug">
  .container-fluid.h-100(v-loading.fullscreen.lock="fullscreenLoading"
    ...
    a-scene
      a-assets(timeout='3000')
        ...
        img(v-for='(item, index) in gameList', crossorigin='anonymous', :id='"R"+index', :key='index', :src='item.Images')
        img(v-for='(item, index) in gameList', crossorigin='anonymous', :id='"S"+index', :key='index', :src='item.Images2')
      a-sky#image-360(v-if='loaded', radius='60', :src='stageBackground')
      a-entity#stage_1(v-if='loaded && stage==1', position='0 -3 0')
        a-entity(v-for='(item, index) in gameList', :id='index', :position='doorPosition[Number(index)]', :rotation='doorRotation[Number(index)]', roomclick='')
      ...
      a-entity#stage_2(v-if='loaded && stage==2', position='0 -3 0')
        a-entity(v-for='(item, index) in slotMachineList', :id='index', :sn='item.Sn', position='0 -1 0', :rotation='"0 "+(Number(index)*30)+" 0"', slotclick='', shadow='')
          //- a-collada-model(src='#slotmodel', position='0 0 -13', rotation='0 0 0', material='color:lavenderblush; metalness:1; roughness: 0; sphericalEnvMap: #pink;')
...
const socketForSeat = io.connect('https://proxy.tgxstatic.info');
let loop;
export default {
  components: {
    tgCashier4,
  ...
  methods: {
    cashierDialog_3_Hide() {
      ...
      console.log('cashierDialog_3_Hide');
    ...
    AFRAMEinit() {
      ...
      // document.querySelector('#slotmodel').addEventListener('model-loaded', () => {
      //   console.log('loaded');
      // });
    },
    init() {
      ...
      // document.getElementsByTagName("body").classList.remove("a-body");
      window.document.body.className = '';
      window.document.documentElement.className = '';
      console.log('initinitinitinitinitinitinitinitinitinit');
      // window.document.getElementsByTagName("html").classList.remove("a-html");
