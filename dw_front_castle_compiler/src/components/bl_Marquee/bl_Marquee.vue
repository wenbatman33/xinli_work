  ...
  .d-flex.Marquee.flex-row.w-100
    ...
    .info.flex-grow-1
      ...
      transition(name="slide-fade")
        p.newsButton(v-for='(item, index) in getIndexNews', v-if='index===currentNum',:key='index', @click='openNewsDialog(item, index)') {{item.newsTitle}}
</template>
<script>
let loop;
export default {
  ...
  methods: {
    ...
    openNewsDialog(item, index) {
      this.$matomo_news(index.newsTitle);
