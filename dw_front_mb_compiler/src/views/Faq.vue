<template lang="pug">
  .faq(v-if="type === 1")
    .faq__bg(:style="{ backgroundImage: 'url(' + (DarkMode ? Image_night : Image_light) + ')' }")
      .faq__bg__header
        router-link(to="/")
          i.fas.fa-chevron-left
        p 帮助中心
      .faq__bg__search
        el-input(type="text" placeholder='请输入关键字，如：存款' v-model="searchText" @change="search")
        i.fas.fa-search.position-absolute.pl-2
      .faq__bg__remind 仍需要帮助？</br>我们提供 7x24 小时的
  ...
  .faqContent(v-else-if="type === 2")
    .faqContent__header(:style="{'backgroundImage': 'url(' + (DarkMode ? bgImage_night : bgImage_light) + ')'}")
  ...
  .faqContent(v-else-if="type === 3")
    .faqContent__header(:style="{'backgroundImage': 'url(' + (DarkMode ? bgImage_night : bgImage_light) + ')'}")
      ...
      p 搜索结果
    .faqContent__main
      p.faqContent__main__title 搜索与"{{ searchText }}"相关的问题：
      .faqContent__main__noData(v-if="searchDate.length === 0")
        p 请联系客服
  ...
  .faqDetail(v-else-if="type === 4")
    .faqDetail__header(:style="{'backgroundImage': 'url(' + (DarkMode ? bgImage_night : bgImage_light) + ')'}")
      .faqDetail__header__back(@click="type = 2")
        i.fas.fa-times
      p {{ detailTitle }}
    .faqDetail__main
      .faqDetail__main__title {{ detailDate.q }}
      .faqDetail__main__content(v-html='detailDate.a ')
...
export default {
  data() {
    return {
      ...
      Image_light: `${window.CDN_ASSETS}/static/img/headerBg_light.png`,
      Image_night: `${window.CDN_ASSETS}/static/img/headerBg_night.png`,
      type: 1,
      // select
      selectTitle: '',
      selectDate: [],
