  ...
  .money
    ...
    .deposit__main(v-if="activeTagName === 'Deposit'")
      div(v-if="depositStep === 1")
        ...
        //- div 
        //-   p.m-0.text-danger(v-for='item in testToken') {{item}}
      ...
      .deposit__main__step2(v-else-if="depositStep === 2 && returnType === 'qrcode'")
        ...
        p.text-center(:class='{"text-white":DarkMode}') 请使用 <span class="text-danger">银联</span> 扫描二维码
      div(v-else-if="depositStep === 2 && returnType === 'form'")
    ...
    .withdraw__main(v-else-if="activeTagName === 'withdraw'")
      ...
      el-select(v-model="withdrawForm.bank_card_id" placeholder="请选择")
        el-option(v-for="item in IndexBankCard"
                  ...
                  :key="item.id"
                  :label="`${filterBankList(item.memberBankCode)} - ${item.memberBankNo}`"
                  :value="item.id")
...
export default {
  ...
  methods: {
    ...
    depositChannel(item, index) {
      if (this.IndexProfile.memberRealName === 0 && item.service_id === 3) {
        this.$message({ message: '请先完善个人资料的真实姓名,如果真实姓名与存款人姓名不符,恐会有延迟到帐问题。', type: 'error' });
      ...
      this.$store.dispatch('payment/API_Deposit', this.depositForm).then((res) => {
        ...
        if (res.data.data.type === 'redirect') {
          ...
          if (window.navigator.userAgent.indexOf('Android') > 0) {
            try {
              window.android.deposit_redirect(res.data.data.content);
            } catch (error) {
              console.log(error);
            }
          } else {
            try {
              window.webkit.messageHandlers.deposit_redirect.postMessage(res.data.data.content);
            } catch (error) {
              console.log(error);
            }
          }
          ...
          const script = '<script>document.forms.postSubmit.submit();<\/script>';

          if (window.navigator.userAgent.indexOf('Android') > 0) {
            try {
              window.android.deposit_form(this.returnContent + script);
            } catch (error) {
              console.log(error);
            }
          } else {
            try {
              window.webkit.messageHandlers.deposit_form.postMessage(this.returnContent + script);
            } catch (error) {
              console.log(error);
            }
          }
  ...
  mounted() {
    if (this.$route.query.token !== undefined && this.$route.query.finger !== undefined) {
      this.testToken.push(this.$route.query.finger);
      // 黑夜以及白天模式
    ...
    window.deposit_redirect = this.deposit_redirect;
    window.deposit_form = this.deposit_form;
  },
  beforeDestroy() {
    window.localStorage.removeItem('JWT_TOKEN');
    window.localStorage.removeItem('Finger');
