...
.AgencyForm
      ...
      .applyAgencyForm
        el-form(:model="applyAgencyForm"  :rules="applyAgencyFormRules" ref="registerForm")
          ...
          el-form-item.col-12.formItem
            el-button.btn.w-100(type='primary', @click.prevent='submit("registerForm")')
              span 注册
          .col-12.formItem
            el-button.btn.w-100(type='primary', @click='gotoIndex')
              span 返回首页
  .AgencyFormStep2(v-if='AgencyFormStep==2')
    ...
    .col-12.my-5
      el-button.btn.w-50(type='primary', @click='gotoIndex')
        span 返回首页
    ...
    submit(form) {
      ...
      this.$refs.registerForm.validate((valid) => {
        if (valid) {
          ...
          axios
            .post(
              `${process.env.APIHOST}/api/Agency/AgencyApply`,
...
table {
  color: #fff;
  width: 100%;
  border: 1px solid $DW_ColorLight;
...
.AgencyTableLine {
  ...
  width: 350px;
  margin: auto;
  border-top: 1px solid $DW_ColorLight;
  border-bottom: 1px solid $DW_ColorLight;
}
.fieldDesc {
  // position: absolute;
  margin-top: -30px;
