<template>
	<div class="container pb-4 FLEX_C">
		<div v-if="showState === 1">
			<div class="container flex content-end justify-center mt-2">
				<div class="grid grid-flow-col grid-rows-2 p-2 my-2 border-2 rounded-md FLEX_C border-primaryLight w-80 btnBg">
					...
					<div class="col-span-2 font-bold text-primary">{{ $t('wallet_withdrawalUSDT_02') }}</div>
			...
			<div class="container flex justify-center p-2">
				<ul class="divide-y-2 divide-[#474747] w-80 bg-[#191919] rounded-lg">
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_03') }}</span>
						<span class="text-white"> 197,473.00 </span>
					</li>
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_04') }}</span>
						<span class="text-white"> TRC 20 </span>
					</li>
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_05') }}</span>
						<div class="grid grid-cols-1">
							<span class="col-span-1 text-xs text-white break-words w-36"> 0xd004a6bc11223180da7 </span>
							<span class="col-span-1 text-xs text-white break-words w-36"> e3fc6670dc970f421b70 </span>
						</div>
					</li>
				</ul>
			</div>
			<div class="container p-4 mt-8 FLEX_C">
				<van-form @submit="onPasswordSubmit">
					<van-field
						v-model="password"
						name="password"
						:label="$t('wallet_withdrawalUSDT_06')"
						:placeholder="$t('wallet_withdrawalUSDT_07')"
						:rules="[{ validator: asyncValidator, message: $t('wallet_withdrawalUSDT_08') }]"
					>
						<template #button><span class="text-[#967140]"> 1/3 </span></template>
					</van-field>

					<van-button class="mt-4" block round type="info" native-type="submit">
						{{ $t('wallet_withdrawalUSDT_09') }}
					</van-button>
					<van-button round plain class="mt-4 mb-6" block color="#C60000" @click="showCancel = true">
						{{ $t('wallet_withdrawalUSDT_10') }}
					</van-button>
				</van-form>
			</div>
		</div>
		<div v-if="showState === 2">
			<div class="grid p-8 m-5 mb-2 gap-x-0.5 rounded-md bg-[#191919]">
				<div class="col-span-1 CENTER_C">
					<span class="text-[#cda269]">{{ $t('wallet_withdrawalUSDT_11') }}</span>
				</div>
			</div>
			...
			<div class="container flex justify-center p-2">
				<ul class="divide-y-2 divide-[#474747] w-80 bg-[#191919] rounded-lg">
					...
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_02') }}</span>
					...
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_12') }}</span>
						<span class="text-white"> 2022/02/06 22:30:00 </span>
					</li>
					<li class="flex justify-between p-3">
						<span class="text-white CENTER_C">{{ $t('wallet_withdrawalUSDT_13') }}</span>
		...
		<div v-if="showState === 3">
			<div class="grid p-8 m-5 mb-2 gap-x-0.5 rounded-md bg-[#191919]">
				<div class="col-span-1 CENTER_C">
					<span class="text-red">{{ $t('wallet_withdrawalUSDT_14') }}</span>
				</div>
			</div>
			<div class="grid grid-cols-1 mt-8">
				...
				<div class="col-span-1 mt-4 CENTER_C">
					<span class="text-[#CDA269]">{{ $t('wallet_withdrawalUSDT_15') }}</span>
				</div>
				<div class="col-span-1 CENTER_C">
					<span class="text-[#CDA269]">{{ $t('wallet_withdrawalUSDT_16') }}</span>
				</div>
			</div>
			<div class="container p-8 mt-8 FLEX_C">
				<van-button class="mt-4" block round type="info" @click="back">
					{{ $t('wallet_withdrawalUSDT_17') }}
				</van-button>
			</div>
		</div>
	</div>
	...
	export default {
		setup() {
			...
			const handleCopy = (value) => {
				...
				if (success) {
					Toast(t('wallet_withdrawalUSDT_18'));
				} else {
					Toast('Fail');
				}
			};
				...
				tableData: [
					{
						...
						status: t('wallet_withdrawalUSDT_19'),
					},
					{
						name: 'Johnnywang',
						money: '999,999,999.00',
