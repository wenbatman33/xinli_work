			...
			<van-list v-model:loading="loading" :finished="finished" :finished-text="$t('more_announcement_02')" @load="onLoad">
				<div v-for="item in tableDataRef" :key="item.title">
					<div class="bg-[#0f0f0f] container px-2 py-4 FLEX_C">
						<van-image v-bind:src="apiImgPath(item.imageH5)" />
...
<script setup lang="ts">
	...
	import type { AGENCYAJAX_AgencyAnnouncementListResponseData } from '@/api/model';
	import { getAgencyAnnouncementList } from '@/api';

	const tableDataRef = ref<Array<AGENCYAJAX_AgencyAnnouncementListResponseData>>([]);
	...
	onMounted(() => {
		getAgencyAnnouncementList().then((res) => {
			loading.value = true;
