 ...
 * 金額區間報表
 */
export const withdrawAmountRangeRepportColumns: BasicColumn[] = [
  {
    title: '日期',
 ...
 * 人數報表
 */
export const withdrawPeopleRepportColumns: BasicColumn[] = [
  {
    title: '日期',
    dataIndex: 'reportDate',
    format: (_text, record) => {
  ...
  },
  {
    ...
    format: (_text, record) => {
      try {
        return Number(record.withdrawNum || '0').toLocaleString();
      } catch (e) {
        return record.withdrawNum;
      }
    },
  },
  {
