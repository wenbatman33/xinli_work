  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const state = reactive<State>({
        showCreate: false,
        tableList: { count: 0, amount: 0, fee: 0 },
        methods: [],
        payChannels: [],
        payChannelNames: [],
        payChannelServices: [],
      ...
      const title = computed<string>(() => {
        return `Records共计${state.tableList.count}笔，订单金额Total Amount ${state.tableList.amount}；交易手续费Fee ${state.tableList.fee}`;
      });
      ...
      const formConfig = computed<FormProps>(() => {
        return {
          ...
          autoAdvancedLine: 1,
          schemas: depositSchemas(state.methods, state.payChannels, state.payChannelNames),
          baseColProps: {
            style: {
              padding: '0px 4px',
            },
      ...
      const [registerTable, { getForm, reload }] = useTable({
        ...
        formConfig,
        api: async (arg) => {
          const query = Object.assign({ source: 2 }, arg);
          ...
          state.tableList.count = response.count || 0;
          state.tableList.amount = response.amount || 0;
          state.tableList.fee = response.fee || 0;
