...
<script lang="ts" setup>
  ...
  import { getDayList, setDayNote } from '/@/api/report/daily';
  ...
  const [registerTable1, { reload, getForm, getDataSource, setProps: setTable1Props }] = useTable({
    ...
    api: async (arg) => {
      ...
      // Get report
      const response = (await getDayList(params)).list;
  ...
  const shiftConverter = (params) => {
    if (params.shift === 1){
      params.reportHourStart = 1;
      params.reportHourEnd = 8;
    }
    if (params.shift === 2){
      params.reportHourStart = 9;
      params.reportHourEnd = 16;
    }
    if (params.shift === 3){
      params.reportHourStart = 17;
      params.reportHourEnd = 24;
    }
    return params;
  }
