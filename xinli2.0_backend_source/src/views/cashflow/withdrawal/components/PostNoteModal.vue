...
<script lang="ts">
  ...
  import { postWithdrawalNote } from '/@/api/cashflow/withdrawal';
    ...
    setup(props, { emit }) {
      ...
      const handleOk = async () => {
        try {
          ...
          // Post deposit note
          const response = await postWithdrawalNote({
          ...
          if (response.success) {
            // Notify success
            message.success(`${props.record?.transactionID} 新增备注成功`);
            ...
            // Notify error
            message.error(`${props.record?.transactionID} 新增备注失败`);
          }
        } catch (e) {
