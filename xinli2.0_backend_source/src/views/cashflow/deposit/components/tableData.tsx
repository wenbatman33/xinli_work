...
export const depositColumns: BasicColumn[] = [
  ...
  },
  {
    ...
    format: (_text, record) => {
      if (record.device === 1) {
        return 'WEB';
      } else if (record.device === 2) {
        return 'H5_android';
      } else if (record.device === 4) {
        return 'H5_ios';
      } else if (record.device === 8) {
        return 'pwa_android';
      } else if (record.device === 16) {
        return 'pwa_ios';
      } else {
  ...
  },
  {
    ...
    width: 100,
    slots: {
      customRender: 'payGroup',
    },
  },
  {
    title: '银行卡组别\nBankGroup',
    dataIndex: 'bankcardGroupId',
    width: 100,
    slots: {
      customRender: 'bankcardGroup',
    },
  },
  {
    title: '会员帐号\nMemberAC',
