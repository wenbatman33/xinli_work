<template>
  <PageWrapper dense contentFullHeight fixedHeight>
    ...
    <PayGroupModal @register="editPayGroupModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  const [editPayGroupModal, { openModal: openPayGroupModal }] = useModal();
  ...
  const openNewWindow = (record: EditRecordRow) => {
    ...
    const newUrl = router.resolve({
      ...
      query: { status: 1, bankGroupID: String(record.ID) },
    });
...
<style lang="scss" scoped>
  ::v-deep(.ant-tooltip-content > .ant-tooltip-inner > div) {
