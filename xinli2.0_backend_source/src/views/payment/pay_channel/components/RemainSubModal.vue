...
<script lang="ts">
  ...
  import { postRemainSub, getPayChannelSearch } from '/@/api/payment/payChannel';
  import { PayChannelItem } from '/@/api/payment/model/payChannelModel';
  import { BasicModal } from '/@/components/Modal';
  import { BasicForm, useForm } from '/@/components/Form';
  import { remainSubSchemas } from './formSchemas';
    ...
    setup(props, { emit }) {
      ...
      const handleOk = async () => {
        try {
          ...
          // Remain sub
          const response = await postRemainSub({
            id: props.record?.id,
          ...
          if (response.success) {
            ...
            message.success(`${props.record?.sn} 结算成功`);
            ...
            // Notify error
            message.error(`${props.record?.sn} 结算失败`);
          }
        } catch (e) {
