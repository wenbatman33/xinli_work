$( document ).ready( function () {
	...
	$( "#forgetFrom" ).validate( {
		...
		submitHandler: function () {
			var formData = $( "#forgetFrom" ).serializeObject();
			$( '#forgetButton' ).prop( 'disabled', 'disabled' );
			$.ajax( {
				type: 'POST',
				contentType: 'application/json; charset=utf-8',
				dataType: 'json',
				url: apiBaseUrl + '/api/forget',
				headers: {
					"X-Is-Mobile": isMobileDevice(),
				},
				data: JSON.stringify( formData ),
				success: function ( res ) {
					var redirectUrl = res.url + '?phone='+ formData.phone +'&data=' + res.data + ( urlQuery ? '&' + urlQuery : '' );
					window.location.replace( redirectUrl );
				},
				error: function ( xhr ) {
					var err = JSON.parse( xhr.responseText );
					$( '.phoneError' ).html( err.message );
					setTimeout( () => { $( '.phoneError' ).html( '' ); }, 3000 );
				}
				,
				complete: function () {
	...
	$( '#forgetFrom input' ).on( 'keyup blur', function () {
