      ...
      el-form-item(label="位置", prop="banner_group_id")
        ...
        el-tooltip
          ...
          template(slot="content")
            p PC:
            p.ml-2.ml-2 主页：档案限制 200KB，尺寸 1388 x 600
            p.ml-2 注册完成页：档案限制 100KB，尺寸 590 x 184
            p.ml-2 登入影片：档案限制 150KB，尺寸 872 x 900
            p.ml-2 盖版广告与游戏介绍页无作用
            el-divider
            p Mobile:
            p.ml-2 主页：档案限制 200KB，尺寸 1146 x 576
            p.ml-2 注册完成页：档案限制 100KB，尺寸 590 x 184
            p.ml-2 登入影片：档案限制 300KB，尺寸 936 x 1634
  ...
  created() {
    ...
    if (this.$route.query.id) {
      getBannerAPI(this.$route.query.id).then(response => {
        ...
        this.insertData.banner_group_id = banner.bannerGroupId ? banner.bannerGroupId.value : 1;
        this.insertData.title = banner.bannerTitle;
        this.insertData.banner_type = banner.bannerType;
        this.insertData.device = banner.bannerDevice ? banner.bannerDevice.value.split(',') : [];
        if (banner.bannerImagePc) {
          this.imagePcList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerImagePc) });
        }
        if (banner.bannerImageMobile) {
          this.imageMobileList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerImageMobile) });
        }
        if (banner.bannerVideoBgPc) {
          this.videoBgPcList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerVideoBgPc) });
        }
        if (banner.bannerVideoBgMobile) {
          this.videoBgMobileList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerVideoBgMobile) });
        }
        this.insertData.video_pc = banner.bannerVideoPc;
        this.insertData.video_pc2 = banner.bannerVideoPc2;
        this.insertData.video_mobile = banner.bannerVideoMobile;
        this.insertData.video_mobile2 = banner.bannerVideoMobile2;
        this.insertData.link_type = banner.bannerLinkType || 1;
        this.insertData.banner_link_pc = banner.bannerLinkPc || '';
        this.insertData.time = [banner.bannerTimeStart, banner.bannerTimeEnd];
        this.insertData.sort = banner.bannerSort;
        this.insertData.status = banner.bannerStatus ? banner.bannerStatus.value : 0;
        this.insertData.game_group_id = banner.bannerGameGroupId;
        this.insertData.game_id = banner.bannerGameId;
      })
    }
  },
  methods: {
