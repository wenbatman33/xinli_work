  ...
  .container-fluid
    ...
    .row
      el-table(:data="tableData", height="75vh", border, stripe)
        el-table-column(v-if="fieldPerms.transDepThirdOrderId", prop="transDepThirdOrderId", label="单号", align="center", width="200", fixed)
        el-table-column(v-if="fieldPerms.transDepThirdCreatedAt", prop="transDepThirdCreatedAt", label="存款时间", align="center", width="200")
        el-table-column(v-if="fieldPerms.transDepThirdPayType", align="center", prop="transDepThirdPayType", label="存款方式", width="200")
        el-table-column(
        ...
        el-table-column(v-if="fieldPerms.transDepThirdAccount", label="会员帐号", align="center", width="140")
          template(slot-scope="{ row }")
            member-display(
              :memberName="row.transDepThirdAccount.display",
              :memberId="row.transDepThirdAccount.value",
              :groups="row.transDepThirdGroups"
            )
        el-table-column(v-if="fieldPerms.transDepThirdRealName", prop="transDepThirdRealName", label="会员姓名", align="center", width="140")
        ...
        el-table-column(
          v-if="fieldPerms.transDepThirdThirdOrderId",
          prop="transDepThirdThirdOrderId",
          label="三方单号",
          align="center",
          width="180"
        )
        el-table-column(v-if="fieldPerms.transDepThirdNote", prop="transDepThirdNote", label="备注", align="center", show-overflow-tooltip)
        el-table-column(v-if="fieldPerms.transDepThirdStatus", label="状态", align="center")
          template(slot-scope="{ row }")
        ...
        el-table-column(v-if="fieldPerms.transDepThirdSuccessTime", prop="transDepThirdSuccessTime", label="完成时间", align="center", width="200")
        ...
        el-table-column(v-if="fieldPerms.transDepThirdStatus", label="管理", width="140", align="center", fixed="right")
          template(slot-scope="{ row }")
            template(v-if="row.transDepThirdStatus.value === 1")
              el-link.text-primary.m-2(v-if="buttonPerms.finishedTDepositOrder", @click="finish(row)") 完成
              el-link.text-primary.m-2(v-if="buttonPerms.failTDepositOrder", @click="fail(row)") 失败
            el-link.text-primary.m-2(v-if="buttonPerms.showTDepositOrderLog", @click="showLog(row)") 查看记录
            el-link.text-primary.m-2(v-if="buttonPerms.addTDepositOrderNote", @click="addNote(row)") 备注
            el-link.text-primary.m-2(v-if="buttonPerms.adjustTDepositOrder && row.transDepThirdStatus.value === 3", @click="addAdjust(row)") 存单错误
    //- log 抽屉
...
export default {
  ...
  methods: {
    ...
    addAdjust(order) {
      this.adjustDialog.account = order.transDepThirdAccount && order.transDepThirdAccount.display;
      this.adjustDialog.data.order_id = order.transDepThirdOrderId;
