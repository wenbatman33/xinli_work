  ...
  .container-fluid
    ...
    .row
      el-table(:data="tableData", height="75vh", , border, stripe)
        el-table-column(v-if="fieldPerms.transPayAllOrderId", prop="transPayAllOrderId", label="单号", align="center", width="200", fixed)
        el-table-column(v-if="fieldPerms.transPayAllOrderStatus", prop="transPayAllOrderStatus", label="状态", align="center", width="120")
        el-table-column(v-if="fieldPerms.transPayAllCheckedAt", prop="transPayAllCheckedAt", label="申请时间", align="center", width="200")
        ...
        el-table-column(v-if="fieldPerms.transPayAllFee", prop="transPayAllFee", label="手续费", align="center", width="200")
        el-table-column(v-if="fieldPerms.transPayAllSplitList", prop="transPayAllSplitList", align="center", width="200")
          span(slot="header") 已完成出款/<br>总出款单数
        el-table-column(v-if="fieldPerms.transPayAllAccount", label="会员帐号", align="center", width="140")
          template(slot-scope="{ row }")
            member-display(
              :memberName="row.transPayAllAccount.display",
              :memberId="row.transPayAllAccount.value",
              :groups="row.transPayAllGroups"
            )
        el-table-column(v-if="fieldPerms.transPayAllBankCardHolder", prop="transPayAllBankCardHolder", label="姓名", align="center", width="140")
        el-table-column(v-if="fieldPerms.transPayAllBankCardNo", prop="transPayAllBankCardNo", label="银行帐号", align="center", width="200")
        ...
        el-table-column(v-if="fieldPerms.transPayAllUpdatedAt", prop="transPayAllUpdatedAt", label="更新时间", align="center", width="200")
        el-table-column(v-if="fieldPerms.TransWDAllStatus", label="管理", width="200", align="center", fixed="right")
          template(slot-scope="{ row }")
            el-link(v-if="buttonPerms.checkWithdrawalOrder")
              //- MARK: 状态仅有繁体字串可判断 应要有状态码可供判别
              router-link.links.mx-1(
                :to="{ path:'checkWithdrawalOrder', query: { title: `出款单 - ${row.transPayAllOrderId}`, id: row.id } }"
              ) {{ row.transPayAllOrderStatus === '完成' || row.transPayAllOrderStatus === '失败' ? '查看' : '出款' }}
            el-link.text-primary.m-1(v-if="buttonPerms.showWithdrawalOrderLog", @click="showLog(row)") 查看记录
            el-link.text-primary.m-1(v-if="buttonPerms.addWithdrawalOrderNote", @click="addNote(row)") 备注
    //- log 抽屉
...
<script>
import { getWithdrawOrderLogAPI } from '@/api/capitalFlow/withdrawOrder';
import {
  getWithdrawalOrdersAPI,
  editWithdrawalOrderAPI
