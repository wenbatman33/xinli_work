<template lang="pug">
  .container-fluid
    .d-flex.justify-content-end(v-if="buttonPerms.checkAgencyWithdrawalOrderFailAll")
      el-button(v-if="order.TransWDAllStatus.value !== 2 && canFailAll", type="danger", @click="failAll") 失败
    //- 提款單內容
    el-card.my-2(shadow="always", v-if="buttonPerms.checkAgencyWithdrawalOrderDetailBlock")
      ...
      .d-flex
        .w-25 单笔提款限额： {{ withdrawOrderInfo.singleMin | formatMoney }} ~ {{ withdrawOrderInfo.singleMax | formatMoney}}
        .w-25 单日提款上限： {{ withdrawOrderInfo.dayMax | formatMoney }}
        .w-25 提款限制： {{ withdrawOrderInfo.withdrawLimit | formatMoney }}
      .d-flex
    ...
    el-card.mt-2(v-for="info in infoList", :key="info.transPayAllSplitId")
      .d-flex.justify-content-between
        ...
        div(style="width: 100px")
          ...
          template(v-else-if="info.transPayAllSplitStatus && info.transPayAllSplitStatus.value < 3")
            el-button.h-75.mb-2.w-100(v-if="buttonPerms.checkAgencyWithdrawalOrderRun", @click="run(info)") 出款
            el-button.h-25.ml-0.w-100(v-if="buttonPerms.checkAgencyWithdrawalOrderFail", @click="fail(info)", :style="{ 'padding': 0 }") 失败
          template(v-else-if="info.transPayAllSplitStatus && info.transPayAllSplitStatus.value === 4")
            el-button.h-100(
              v-if="buttonPerms.checkAgencyWithdrawalOrderReRun && order.TransWDAllStatus.value !== 2",
              @click="reRun(info)"
            ) 重新出款
</template>
    ...
    if (this.$route.query.id) {
      this.getOrder().then(() => {
        ...
        getWithdrawOrderInfoAPI(this.order.TransWDAllOrderId).then(response => {
          this.withdrawOrderInfo = response.data.data;
        });
      });
  ...
  methods: {
    ...
    failAll() {
      this.$swal({
        ...
        this.getOrder().then(() => {
          this.getInfo();
          this.$swal('成功！', '此提款单已设为失败', 'success');
        });
      }).catch(() => {});
    },
    reRun(info) {
