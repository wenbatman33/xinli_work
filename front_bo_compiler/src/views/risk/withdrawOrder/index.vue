      ...
      el-table(:data="tableData", height="75vh", border, stripe)
        ...
        el-table-column(v-if="fieldPerms.TransWDAllStatus", label="状态", align="center", width="120")
          template(slot-scope="{ row }")
            ...
            span(
              v-if="fieldPerms.TransWDAllFinanceCheck && row.TransWDAllFinanceCheck.value !== 0"
            ) <br>财务{{ row.TransWDAllFinanceCheck.display }}
        ...
        el-table-column(v-if="fieldPerms.TransWDAllAccount", label="会员帐号", align="center", width="140")
          template(slot-scope="{ row }")
            template(v-if="row.TransWDAllAccount")
              router-link(
                v-if="fieldPerms.TransWDAllMemberId",
                :to="{ path: '/memberDetail', query: { title: row.TransWDAllAccount, id: row.TransWDAllMemberId } }"
              ) {{ row.TransWDAllAccount }}
              span(v-else) {{ row.TransWDAllAccount }}
            span(v-else) 查无玩家
        el-table-column(v-if="fieldPerms.TransWDAllRealName", prop="TransWDAllRealName", label="会员姓名", align="center", width="140")
        el-table-column(v-if="fieldPerms.TransWDAllMemberGroup", label="群组", align="center", width="200")
          template(slot-scope="{ row }")
            el-tooltip(v-if="row.TransWDAllMemberGroup.length > 3")
              div(slot="content")
                el-tag.m-1(v-for="(group, index) in row.TransWDAllMemberGroup", :key="`${row.id}-${index}`") {{ group }}
              el-badge.mt-2(:value="row.TransWDAllMemberGroup.length", type="warning")
                el-tag {{ row.TransWDAllMemberGroup[0] }}
            el-tag.m-1(v-else, v-for="(group, index) in row.TransWDAllMemberGroup", :key="`t${row.id}-${index}`") {{ group }}
        ...
        el-table-column(v-if="fieldPerms.TransWDAllStatus", label="管理", width="200", align="center", fixed="right")
          template(slot-scope="{ row }")
            el-link(v-if="buttonPerms.checkRiskWithdrawOrder && row.TransWDAllAccount")
              router-link.links.mx-1(
                :to="{ path:'riskCheckWithdrawOrder', query: { title: `提款单 - ${row.TransWDAllOrderId}`, id: row.id } }"
              ) {{ row.TransWDAllStatus.value === 1 ? '审核' : '查看' }}
            el-link.text-primary.m-1(v-if="buttonPerms.showRiskWithdrawOrderLog", @click="showLog(row)") 查看记录
            el-link.text-primary.m-1(v-if="buttonPerms.addRiskWithdrawOrderNote", @click="addNote(row)") 备注
    //- log 抽屉
...
export default {
  ...
  methods: {
    getData() {
      ...
      getRiskWithdrawOrdersAPI(APIParams).then(response => {
