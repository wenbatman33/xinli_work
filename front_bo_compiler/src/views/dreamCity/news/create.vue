  ...
  created() {
    if (this.$route.query.id) {
      getNewsAPI(this.$route.query.id).then(response => {
        ...
        this.insertData.device = news.newsMallDevice ? news.newsMallDevice.value.split(',') : [];
        this.insertData.time = [news.newsMallTimeStart, news.newsMallTimeEnd];
        this.insertData.title = news.newsMallTitle;
        this.insertData.content = news.newsMallContent;
        this.insertData.top = news.newsMallTop ? news.newsMallTop.value : 0;
        this.insertData.status = news.newsMallStatus ? news.newsMallStatus.value : 0;
      })
    }
  },
  methods: {
