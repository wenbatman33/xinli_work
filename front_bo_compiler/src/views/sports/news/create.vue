  ...
  created() {
    if (this.$route.query.id) {
      getNewsAPI(this.$route.query.id).then(response => {
        ...
        this.insertData.device = news.newsPEDevice ? news.newsPEDevice.value.split(',') : [];
        this.insertData.time = [news.newsPETimeStart, news.newsPETimeEnd];
        this.insertData.title = news.newsPETitle;
        this.insertData.content = news.newsPEContent;
        this.insertData.top = news.newsPETop ? news.newsPETop.value : 0;
        this.insertData.status = news.newsPEStatus ? news.newsPEStatus.value : 0;
      })
    }
  },
  methods: {
