  ...
  created() {
    ...
    if (this.$route.query.id) {
      getBannerAPI(this.$route.query.id).then(response => {
        ...
        this.insertData.banner_group_id = banner.bannerPEGroupId ? banner.bannerPEGroupId.value : 1;
        this.insertData.title = banner.bannerPETitle;
        this.insertData.banner_type = banner.bannerPEType;
        this.insertData.device = banner.bannerPEDevice ? banner.bannerPEDevice.value.split(',') : [];
        if (banner.bannerPEImagePc) {
          this.imagePcList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerPEImagePc) });
        }
        if (banner.bannerPEImageMobile) {
          this.imageMobileList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerPEImageMobile) });
        }
        if (banner.bannerPEVideoBgPc) {
          this.videoBgPcList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerPEVideoBgPc) });
        }
        if (banner.bannerPEVideoBgMobile) {
          this.videoBgMobileList.push({ url: this.$options.filters.urlAddTimestamp(banner.bannerPEVideoBgMobile) });
        }
        this.insertData.video_pc = banner.bannerPEVideoPc;
        this.insertData.video_pc2 = banner.bannerPEVideoPc2;
        this.insertData.video_mobile = banner.bannerPEVideoMobile;
        this.insertData.video_mobile2 = banner.bannerPEVideoMobile2;
        this.insertData.link_type = banner.bannerPELinkType || 1;
        this.insertData.banner_link_pc = banner.bannerPELinkPc || '';
        this.insertData.time = [banner.bannerPETimeStart, banner.bannerPETimeEnd];
        this.insertData.sort = banner.bannerPESort;
        this.insertData.status = banner.bannerPEStatus ? banner.bannerPEStatus.value : 0;
        this.insertData.game_group_id = banner.bannerPEGameGroupId;
        this.insertData.game_id = banner.bannerPEGameId;
      })
    }
  },
  methods: {
