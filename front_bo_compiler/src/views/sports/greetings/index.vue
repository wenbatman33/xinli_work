<template lang="pug">
  .container-fluid
    data-filter(ref="dataFilter", :filterList="filterList", @change="getData()")
      el-button.ml-2(v-if="buttonPerms.sportAddGreetings", @click="$router.push('/sportGreetingsEdit?title=新增欢迎词')") 新增欢迎词
    //- 分页 表格上方
    .justify-content-end
      el-pagination(
        :background="true",
    ...
    .row
      el-table(:data="tableData", height="75vh")
        ...
        el-table-column(label="操作", width="200", align="center")
          template(slot-scope="{ row }")
            .text-left
              el-link.text-danger(v-if="buttonPerms.sportDeleteGreetings", @click="deleteGreetings(row)") 删除
              template(v-if="buttonPerms.sportEditGreetingsStatus && fieldPerms.greetStatus")
...
export default {
  ...
  created() {
    ...
    // this.getData();
    this.$swal('体育API未完成', '请先至全站欢迎词', 'warning');
