<template lang="pug">
  .container-fluid
    ...
    .row
      el-table(:data="pushManagePEList", style="width: 100%")
        el-table-column(v-if="fieldPerms.id", prop="id", label="编号", align="center")
        el-table-column(v-if="fieldPerms.pushManagePEReserviePushDatetime", label="推播时间", align="center")
          template(slot-scope="{ row }")
            span(v-if="row.pushManagePEReserviePushDatetime === '0000-00-00 00:00:00'") 立即
            span(v-else) {{ row.pushManagePEReserviePushDatetime }}
        el-table-column(v-if="fieldPerms.pushManagePEPushDateTime", prop="pushManagePEPushDateTime", label="系统推播时间", align="center")
        el-table-column(v-if="fieldPerms.pushManagePEPushType", prop="pushManagePEPushType.display", label="类型", align="center")
        el-table-column(v-if="fieldPerms.pushManagePEPushMessage", prop="pushManagePEPushMessage", label="讯息", align="center", show-overflow-tooltip)
        el-table-column(v-if="fieldPerms.pushManagePEPushPosition", prop="pushManagePEPushPosition", label="连结", align="center", show-overflow-tooltip)
        el-table-column(v-if="fieldPerms.pushManagePEPushDevice", prop="pushManagePEPushDevice", label="装置", align="center")
          template(slot-scope="{ row }")
            el-tag.m-1(type="success", v-for='(item, key) in row.pushManagePEPushDevice') {{item.display}}
        el-table-column(v-if="fieldPerms.pushManagePEUpdatedUser", prop="pushManagePEUpdatedUser.display", label="操作人", align="center")
        el-table-column(v-if="fieldPerms.pushManagePEPushStatus", prop="pushManagePEPushStatus.display", label="状态", align="center")
        el-table-column(label="管理", width="200", align="center")
          template(slot-scope="{ row }")
            .text-left
              //- 待派發狀態
              template(v-if="row.pushManagePEPushStatus && row.pushManagePEPushStatus.value === 0")
              ...
              template(v-if="buttonPerms.sportPushManageHasLogNeedId")
                span |
  ...
  methods: {
    getData() {
      ...
      getPushManageAPI(APIParams).then(response => {
        this.pushManagePEList = response.data.data
      })
    },
    addPushManageItem(){
      // 清空表單
    ...
    editPushManageItem(row){
      ...
      this.$set(this.formData, 'push_time_type', row.pushManagePEPushTimeType.value);
      this.$set(this.formData, 'reservie_push_datetime', row.pushManagePEReserviePushDatetime);
      this.$set(this.formData, 'push_type', row.pushManagePEPushType.value);
      this.$set(this.formData, 'push_device', Object.values(row.pushManagePEPushDevice).map(item => item.value));
      this.$set(this.formData, 'push_message', row.pushManagePEPushMessage);

      this.$set(this.formData, 'push_target_type', row.pushManagePEPushTargetType.value);
      if (this.formData.push_target_type === 2) {
        const target = row.pushManagePEPushTarget.split(',').map(Number);
        ...
        this.$set(this.formData, 'push_target', row.pushManagePEPushTarget);
      }
      this.$set(this.formData, 'push_position_type', row.pushManagePEPushPositionType.value);
      this.$set(this.formData, 'push_position', row.pushManagePEPushPosition);
      this.$set(this.formData, 'pushManagePushStatus', row.pushManagePEPushStatus.value);
      this.$set(this.formData, 'pushManageStiatisticsReport', Object.values(row.pushManagePEStiatisticsReport).map(item => item.messageId));
      this.$set(this.formData, 'status', row.pushManagePEStatus.value);
