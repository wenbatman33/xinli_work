...
$.ajax({
  type: "GET",
  dataType: "json",
  url: 'https://test.blstatic.space/upload/domains_json/domains.json?'+Date.now(),
  success: function(Jdata) {
    Jdata.map(function(i, index){
      var item;
      item='<li class="template m-3">';
      item+='<a class="linkATag url_name d-flex" href="'+i.link+'">';
...
function speedTime(link, index){
  ...
  var endTime;
  var img = new Image();
  img.src = link+'/static/img/logo.png?'+startTime;
  img.onload = function(){
    times += 1; 
    endTime = Date.now();
