  ...
  .container-fluid
    .row.no-gutters.px-3.py-3.border-bottom
      ...
      .col-6.d-flex.justify-content-end
        el-button(@click='newDialog = true', size="medium", type='primary') 新增主群組
    ...
    .row.no-gutters.px-3
      template
        el-table(:data='filter_table_data', border='', style='width: 100%')
          ...
          el-table-column(prop='Name', label='主群組名稱', align='center')
          el-table-column(prop='Status_str', label='狀態', align='center')
          el-table-column(label='操作', width='200')
            template(slot-scope='scope')
              a.links(@click='get_group(scope.row.Sn)', type='text') 修改
              router-link.links(:to="{name:'tgSubFunctionGroup', query:{main_group_id: scope.row.Sn}}") 次群組列表
              router-link.links(:to="{name:'tgAuthorityList', query:{main_group_id: scope.row.Sn}}") 權限管理
    //--Dialog
    el-dialog(title='新增主群組', :visible.sync='newDialog', @close="init_dialog('add_form')")
      el-form.mr-3.form-style(:model="form", :rules="rules", ref="add_form", status-icon)
        el-form-item(label='主群組名稱', prop="group_name", label-width="100px" ,label-position='left')
          el-input(v-model='form.group_name', placeholder='輸入主群組名稱', clearable)
        el-form-item(label='排序', prop="sort", label-width="100px" ,label-position='left', clearable)
          el-input(v-model='form.sort', type="number", min="0" placeholder='輸入排序')
    ...
    //--Dialog
    el-dialog(title='修改主群組', :visible.sync='updateDialog', @close="init_dialog('update_form')")
    ...
    add_group() {
      ...
      this.$refs["add_form"].validate(valid => {
        if (valid) {
          ...
          axios({
            method: "post",
            url: process.env.VUE_APP_APIHOST + "Admin/MainFunctionManage/Add",
            data: {
    ...
    update_group() {
      ...
      this.$refs["update_form"].validate(valid => {
        if (valid) {
          ...
          axios({
            method: "post",
            url: process.env.VUE_APP_APIHOST + "Admin/MainFunctionManage/Edit",
            data: {
              Sn: self.update_id,
              Name: self.form.group_name,
              Sort: self.form.sort,
