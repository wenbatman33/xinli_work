        ...
        .row.no-gutters
          .col-md-2.col-12.px-1.py-1
            el-radio-group(v-model='tableSwitch', size='medium', @change='changeSwitch')
              el-radio-button.radio-style(label=1) 存款報表
          ...
          .col-md-3.col-12.px-1.py-1
            el-select(size='medium', v-model='search_pay_channel_id', filterable='', placeholder='選擇存款方式')
              el-option(key='', label='不指定存款方式', value='')
              el-option(v-for='(item, index) in typeoption', :key='index', :label='item.ChannelName', :value='index')
      ...
      .col-12
        template
          el-table(v-if='tableSwitch == 1' :data='tableData', stripe='', border='', show-summary='', :summary-method="getSummaries", style='width:100%')
            ...
            el-table-column(prop='OnlyPeople', label='唯一存款人數(人)', align='right')
              template(slot-scope='scope')
                span {{scope.row.OnlyPeople | commaFormat}}
            el-table-column(prop='FirstPeople', label='新增存款人數(人)', align='right')
              template(slot-scope='scope')
                span {{scope.row.FirstPeople | commaFormat}}
            el-table-column(prop='PayCount', label='存款次數(次)', align='right')
              template(slot-scope='scope')
                span {{scope.row.PayCount | commaFormat}}
            el-table-column(prop='Amount', label='存款金額(¥)', align='right')
              template(slot-scope='scope')
                span {{scope.row.Amount | commaFormat}}
            el-table-column(prop='AvgAmount', label='人均存款金額(¥)', align='right')
              template(slot-scope='scope')
                span {{scope.row.AvgAmount | commaFormat}}
            el-table-column(prop='Fee', label='手續費(¥)', align='right')
              template(slot-scope='scope')
                span {{scope.row.Fee | commaFormat}}
            el-table-column(prop='ActualAmount', label='實收金額(¥)', align='right')
              template(slot-scope='scope')
...
export default {
  ...
  methods: {
    ...
    get_data(now_page) {
      ...
      let payload = {
        ...
        ProductTag: self.search_pay_channel_id,
        Page: now_page,
  ...
  data() {
    return {
      tableSwitch: 1, //1.存款報表 2.筆數區間
      lock_btn: false,
      now_page: 1,
      total_page: 0,
      page_size: "30",
      search_time_range: [],
      search_pay_type: 1, // 1.存款報表 2.提款報表
