    ...
    .row.no-gutters.px-3
      template
        el-table(:data='filter_table_data', border='', style='width: 100%')
          el-table-column(prop='GroupId', label='排序', width='100', align='center')
    ...
    add_group() {
      ...
      this.$refs["add_form"].validate(valid => {
        if (valid) {
          ...
          axios({
            method: "post",
            url: process.env.VUE_APP_APIHOST + "Web/PromotionsGroupManage/Add",
            data: {
    ...
    update_group() {
      ...
      this.$refs["update_form"].validate(valid => {
        if (valid) {
          ...
          axios({
            method: "post",
            url: process.env.VUE_APP_APIHOST + "Web/PromotionsGroupManage/Edit",
            data: {
