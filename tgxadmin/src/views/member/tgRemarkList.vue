  ...
  .container-fluid
    .row.no-gutters.px-3.py-3.border-bottom
      ...
      .col-6.d-flex.justify-content-end
        el-button(size="medium",type='primary', @click='newDialog = true') 新增備註類型
    ...
    .row.no-gutters.px-3
      template
        el-table(ref='multipleTable', :data='filter_table_data', border='', style='width: 100%')
          ...
          el-table-column(prop='GroupName', label='備註類型')
          el-table-column(prop='StatusName', label='狀態', align='center')
          el-table-column(prop='EditUserName', label='建立者', align='center')
          el-table-column(prop='Update_At', label='建立日期', width='170')
          el-table-column(label='操作', width='120')
            template(slot-scope='scope')
              a.links(@click='get_group(scope.row.GroupId)', type='text') 修改
              a.cancel(@click='handleDeleteComfirm(scope.row.GroupId)', type='text') 刪除
    //--Dialog
    el-dialog(title='新增類型', :visible.sync='newDialog', @close="init_dialog('add_form')")
      el-form.mr-3(:model="form", :rules="rules", ref="add_form", status-icon)
        el-form-item(label='類型名稱', prop="group_name", label-width="88px" ,label-position='left')
          el-input(v-model='form.group_name', placeholder='輸入類型名稱', clearable)
    ...
    //--Dialog
    el-dialog(title='修改類型', :visible.sync='updateDialog', @close="init_dialog('update_form')")
  ...
  methods: {
        ...
        if (valid) {
          ...
          axios({
            ...
            url:
              ...
              "Member_All/UserNoteGroupManage/Add",
            data: {
      ...
      this.$refs["update_form"].validate(valid => {
        if (valid) {
          ...
          axios({
            ...
            url:
              process.env.VUE_APP_APIHOST +
              "Member_All/UserNoteGroupManage/Edit",
            data: {
    ...
    del_tag(group_id) {
      ...
      axios({
        method: "post",
        url:
          process.env.VUE_APP_APIHOST + "Member_All/UserNoteGroupManage/Delete",
        data: {
          GroupId: group_id
        }
      })
