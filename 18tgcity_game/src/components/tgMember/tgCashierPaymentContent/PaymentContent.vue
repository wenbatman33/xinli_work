    ...
    el-form(:model="depositForm", :rules="depositFormRules", label-width="80px", ref="depositForm")
      .col-12(v-if='ProductTag==="UnionPay" && Info.Bank !=="" ')
        el-form-item(v-if='ProductTag=="UnionPay"', label="支援銀行") 
          el-tag.mx-1(type='success' v-for='(item, key) in SupportBankCode') {{item.BankName}}
      el-form-item.col-12(label="存款金额", prop="Amount")
        el-input(type='number', v-model="depositForm.Amount", placeholder="请输入存款金额")
...
let popup;
export default {
  props: ['ProductTag', 'SupportBankCode', 'Info', 'Another'],
  data() {
    return {
      nextStep: '',
      BankList: '',
          ...
          axios.post(APIPath, payload)
            .then((res) => {
              ...
              if (res.data.status === 1) {
                ...
                vm.$message({ message: '存款失敗', type: 'error' });
              }
            })
            .catch((error) => {
              console.log(error);
