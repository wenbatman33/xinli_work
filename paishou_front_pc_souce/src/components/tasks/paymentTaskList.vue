<template lang="pug">
.list_box(v-if='basicTaskList.length<=0  && paymentTaskList.length>0')
	.font-black.mb-2.BETWEEN
		span.text-psV3Red 充值任务
	.task_list(v-for='(item, index) in paymentTaskList', :key='index')
		div
			p.taskname {{ item.name }}
			p.text-psV3Yellow.text-xs {{item.bonus_type_name}} +{{ item.quantity }}
	...
	a-modal.radiusModal(v-if='state.showTask', :visible='state.showTask', :width='showTaskType===2?"1000px":"600px"', :bodyStyle='{"padding":0}', :closable='true', :footer='null', @cancel='state.showTask=false')
		.CENTER(class='h-[580px]')
			.p-4.text-center(v-if='state.success')
			...
			.p-4.text-center(v-if='!state.success')
				h3.text-center.text-psV3Red 领取失败
				.py-4
					a-button.rounded(type='primary', size='large', @click='state.showTask=false') 确认
</template>
...
export default {
	setup() {
		...
		const paymentTaskList = computed(() => {
			const List = store.state.tasks.paymentTaskList;
			return List.filter((task) => task.status != 2);
		});
		const todoTask = (link) => {
			deepLink(link);
