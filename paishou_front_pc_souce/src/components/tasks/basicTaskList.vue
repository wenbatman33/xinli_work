<template lang="pug">
.list_box(v-if='basicTaskList.length>0')
	.font-black.mb-2.text-psV3Red  新手任务
	.task_list(v-for='(item, index) in basicTaskList', :key='index')
		div
			p.taskname {{ item.name }}
			p 
				span.text-orange-500 {{item.bonus_type_name}}+{{item.quantity}}
...
export default {
	setup() {
		...
		const getBonus = (item) => {
			...
			if (raw.id === 1) {
				getBonusRegister().then((res) => {
					if (res.code === 0) {
						finish();
					}
				});
			} else if (raw.id === 2) {
				getBonusAvatar().then((res) => {
					if (res.code === 0) {
						finish();
					}
				});
			} else if (raw.id === 3) {
				getBonusPhone().then((res) => {
					if (res.code === 0) {
						finish();
					}
				});
