...
.page.pb-12(class='sm:px-2 md:px-4 lg:px-8 xl:px-20')
	.partTitle.FLEX_R.justify-center.my-8
	...
	.rouletteWrap
		.roulette.flex.justify-center.items-center
			button.playBtn.flex.justify-center.items-center(@click='play', :disabled='state.gamePlayBtnDisabled')
				img(src='/publicAssets/img/event/mon_coming/TurntableBtnText@2x.png', v-if='!state.gamePlayBtnDisabled')
			img.layer.layer_1(src='/publicAssets/img/event/mon_coming/layer_1.png')
			img.layer.layer_2(src='/publicAssets/img/event/mon_coming/layer_2.png')
			img.layerBase.layer_3(src='/publicAssets/img/event/mon_coming/layer_3.png')
	.text-center.my-4
		a-button.greenBtn.cursor-default(type='primary', round) 您目前拥有转盘机会 {{state.count}} 次
	.partTitle.FLEX_R.justify-center.my-8
		img(src='/publicAssets/img/event/mon_coming/img-wheat-left.png', srcset="/publicAssets/img/event/mon_coming/img-wheat-left@2x.png 2x")
		span
	...
	//- ------------------------------------------------------------------------------------------------------------------------------------------------
	a-modal(v-if='state.showGiftModal', :visible='state.showGiftModal', :class='"EventModal"', closable='', :footer='null',  @cancel='state.showGiftModal=false')
		h2.text-psV3Red.text-center.my-2 恭喜抽得
		.text-center.my-4
			img.filterGiftImg(:src='state.currentData.icon')
		.text-center.my-4
			span.text-psV3Red.mx-2 {{state.currentData.name}}
			span.text-psV3Red.mx-2 {{state.quantity}}
		.text-center.my-4
			button.mx-2.confirmBtn(@click='state.showGiftModal=false; play()') 再抽一次
			button.mx-2.cancelBtn(@click='state.showGiftModal=false') 关闭
	//- ------------------------------------------------------------------------------------------------------------------------------------------------
	a-modal(v-if='state.nochanceModal', :visible='state.nochanceModal', :class='"EventModal"', closable='', :footer='null',  @cancel='state.nochanceModal=false')
		h2.text-psV3Red.text-center.my-2 喔欧～您已无转盘机会
		p.text-gray-500.text-center.my-2 是否直接购买转盘机会1次，进行游戏？
		p.text-gray-500.text-center.my-2 (购买1次机会，直接扣20,000蕉币)
		.text-center.my-4
			button.mx-2.confirmBtn(@click='state.nochanceModal=false;') 等等再玩
			button.mx-2.cancelBtn(@click='state.nochanceModal=false; buyCount(1)') 购买一次
</template>
<script>
			...
			giftList: [
				{
					id: '01',
					name: '摩天大厦',
					icon: '/publicAssets/img/event/mon_coming/img-gift-01.png',
				},
				{
					id: '02',
					name: '熔岩火山',
					icon: '/publicAssets/img/event/mon_coming/img-gift-02.png',
				},
				{
					id: '03',
					name: '香蕉',
