...
.grid.grid-cols-item.gap-2.items-center.bg-item.opacity-item.rounded.p-4(:class='clazz')
    ...
    //- 任務狀態
    a-button.w-auto.h-auto.px-2.py-1.rounded-full.bg-status.border-status(type='link', :class='clazz', :loading='state.loading', @click='clickStatus')
        .text-sm.text-status.font-bold(v-if='!state.loading', :class='clazz') {{ status }}
</template>
<script>
