<template lang='pug'>
.tgOverview(v-loading="loading",element-loading-text="加载中")
  h5.px-3 常用钱包
  .row.m-1(v-loading.fullscreen.lock="fullscreenLoading",element-loading-text="拼命加载中")
...
export default {
  ...
  methods: {
    ...
    WalletTransferAll(GroupId, GroupName) {
      ...
      const vm = this;
      // /////////////////////////////
      if (this.$route.path === '/member/overview') {
        this.GA_fastout('wallet_fastout', GroupId);
      } else if (this.$route.path === '/member/cashier/3') {
        this.GA_fastout('cashier_fastout', GroupId);
      } else {
        this.GA_fastout('transfer_fastout', GroupId);
      }
    ...
    WalletTransferGame(GroupId, GroupName) {
      ...
      // /////////////////////////////
      if (this.$route.path === '/member/overview') {
        this.GA_fastin('wallet_fastin', GroupId);
      } else if (this.$route.path === '/member/cashier/3') {
        this.GA_fastin('cashier_fastin', GroupId);
      } else {
        this.GA_fastin('transfer_fastin', GroupId);
      }
    ...
    seletePropsWallet(game, index, GroupName) {
      // /////////////////////////////
      if (this.$route.path === '/member/overview') {
        this.GA_transfer2('wallet_transfer2', index);
      } else if (this.$route.path === '/member/cashier/3') {
        this.GA_transfer2('cashier_transfer2', index);
      } else {
        this.GA_transfer2('transfer_transfer2', index);
      }
      const Wallet_1 = {
        gameId: index,
        gameName: game[0],
