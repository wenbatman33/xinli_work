...
.col-12
  ...
  div(v-if='MemberRecords.MemberWithdrawLog.data_list')
    ...
    el-table(:data="MemberRecords.MemberWithdrawLog.data_list" border style="width: 100%" sortable='')
      ...
      el-table-column(label="狀態" prop="StateCode" sortable='')
        template(slot-scope="scope")
          ...
          el-tooltip(v-if='scope.row.StateCode==3',class="item" effect="dark",:content="scope.row.Note" placement="top")
            span.Note.text-danger {{formatStatus(scope.row.StateCode)}} {{scope.row.Note}}
  tgError(v-else)
  div.mt-1.mb-1.text-right(v-if='getTotalPages>0')
    el-pagination(@current-change="handleCurrentChange"  :current-page.sync="currentPages" background="" layout="prev, pager, next" :total="getTotalPages" :page-size='30')
</template>
...
export default {
  ...
  computed: {
    ...
    getTotalPages() {
      return Number(this.MemberRecords.MemberWithdrawLog.total_rows);
    },
  },
  methods: {
