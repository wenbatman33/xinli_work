...
.col-12
  ...
  div(v-if='MemberBonus.MemberBonusLog.data_list')
    el-table(:data="MemberBonus.MemberBonusLog.data_list" border style="width: 100%" sortable='')
      el-table-column(type="expand")
        template(slot-scope="props")
          el-form.demo-table-expand(label-position="left" inline="")
            el-form-item(label="备注")
              span {{ props.row.Notes2 }}
      el-table-column(label="来源" prop="OfferName"  width='100' sortable='')
      el-table-column(label="日期(GMT＋8)" width='150' prop="CalcDate" sortable='')
      el-table-column(label="存款金额" prop="CalcCash" sortable='')
        template(slot-scope="scope")
          span ￥{{scope.row.CalcCash | commaFormat }}
      el-table-column(label="红利金额" prop="ModifyCash" sortable='')
        template(slot-scope="scope")
          span ￥{{scope.row.ModifyCash | commaFormat }}
      //- el-table-column(label="红利比例" prop="OfferRate"  :formatter="percentageOfferRate"  sortable='')
...
export default {
  ...
  computed: {
    ...
    getTotalPages() {
      return Number(this.MemberBonus.MemberBonusLog.total_rows);
    },
  },
  methods: {
    ...
    ApplyBonusGiveUp(Sn) {
      ...
      axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/BonusGiveUp/bonus`, payload)
        .then((res) => {
    ...
    ApplyOffer(rowData, Sn, Status) {
      ...
      if (Status === 1) {
        axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/GetBonusGameWallet/bonus`, payload)
          .then((res) => {
            console.log(res);
    ...
    ApplyOfferConfirm(payload) {
      const vm = this;
      console.log(payload);
      axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/ApplyOffer/bonus`, payload)
        .then((res) => {
          console.log(res);
          if (res.data.status === 1) {
            vm.getAPI();
