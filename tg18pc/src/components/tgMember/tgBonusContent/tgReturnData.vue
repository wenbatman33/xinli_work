...
.col-12
  ...
  div(v-if='MemberBonus.MemberReturnLog.data_list')
    el-table(:data="MemberBonus.MemberReturnLog.data_list" border style="width: 100%" sortable='')
      ...
      el-table-column(label="流水" prop="CalcCash" sortable='')
        template(slot-scope="scope")
          span ￥{{scope.row.CalcCash | commaFormat }}
      el-table-column(label="返水金額" prop="ModifyCash" sortable='')
        template(slot-scope="scope")
          span ￥{{scope.row.ModifyCash | commaFormat }}
      //- el-table-column(label="返水比例" prop="OfferRate"  :formatter="percentageOfferRate" sortable='')
...
export default {
  ...
  computed: {
    ...
    getTotalPages() {
      return Number(this.MemberBonus.MemberReturnLog.total_rows);
    },
  },
  methods: {
    ...
    ApplyBonusGiveUp(Sn) {
      ...
      axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/BonusGiveUp/return`, payload)
        .then((res) => {
    ...
    ApplyOffer(rowData, Sn, Status) {
      ...
      if (Status === 1) {
        axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/GetBonusGameWallet/return`, payload)
          .then((res) => {
            console.log(res);
    ...
    ApplyOfferConfirm(payload) {
      const vm = this;
      console.log(payload);
      axios.post(`${process.env.VUE_APP_APIHOST}/WalletAll/ApplyOffer/return`, payload)
        .then((res) => {
          console.log(res);
          if (res.data.status === 1) {
            vm.getAPI();
