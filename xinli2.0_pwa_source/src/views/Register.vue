...
.flex.flex-col.flex-grow.justify-between.items-center.pb-20
  ...
  .flex.flex-col.items-stretch.w-80.pt-50px.z-20
    ...
    //- 登入
    BasicButton.mt-8(:loading='registerLoading', :disabled='registerLoading', bg='accent100', size='large', @click='handleRegisterClick') {{ t("register_register") }}
...
<script setup lang="ts">
  ...
  import { getRegisterConfig, getCaptcha, postSSORegister } from '@/api/member/authorize';
  ...
  /**
   * Register button disabled status
   */
  ...
  const updateConfig = async () => {
    try {
      ...
      // Update config
      dxEnabled.value = response?.data?.data?.registerVerify == true;
    } catch (e) {
  ...
  const handleRegisterClick = async () => {
    try {
      ...
      // Post register
      const response = await postSSORegister({
