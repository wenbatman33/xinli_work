...
Popup.h-full.bg-white(:show='show', :safeAreaInsetBottom='true', teleport='body', position='bottom', @update:show='handleUpdateShow')
  ...
  .h-full.grid.grid-rows-content.justify-stretch
    ...
    .flex.flex-col.items-stretch.overflow-y-auto.px-8
      //- 提示
      .body1.text-primary.opacity-40.mt-8 {{ t("personal_qq_remainder") }}

      //- QQ號
      BasicField.mt-6(v-model:value='account', :error='accountError', :placeholder='t("personal_qq_hint")')
...
<script setup lang="ts">
  ...
  const handleConfirmClick = async () => {
    try {
      ...
      if (!/^[0-9]{5,11}$/.test(account.value)) {
        accountError.value = t('personal_qq_formatInvalid');
        return;
      }
