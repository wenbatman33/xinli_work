<template lang="pug">
//- 內開網頁彈窗
Popup.h-full.bg-background(:show='show', :safeAreaInsetBottom='true', teleport='body', position='bottom', @update:show='handleUpdateShow')
  //- 彈窗內容
  .h-full.flex.flex-grow.flex-col.items-stretch
    //- 導覽欄
    BasicNav(:title='title')
      //- 右側
      template(#right)
        //- 關閉
        BasicButton(rounded='full', @click='handleCloseClick')
          img.p-2(:src='iconClosePrimary')

    //- 網頁內容
    iframe.flex-grow(:src='url')
</template>

<script setup lang="ts">
  import { computed } from 'vue';
  import { Popup } from 'vant';
  import { staticImgUrl } from '@/utils';
  import { useAppStore } from '@/store/app-store';
  import BasicNav from '@/components/nav/BasicNav.vue';
  import BasicButton from '@/components/button/BasicButton.vue';

  /**
   * Static icons
   */
  const iconClosePrimary = staticImgUrl('icon-close-primary.svg');
  const iconRefreshPrimary = staticImgUrl('icon-refresh-primary.svg');

  /**
   * App store
   */
  const appStore = useAppStore();

  /**
   * Show
   */
  const show = computed(() => {
    return appStore.getInAppPopupVisible;
  });

  /**
   * Title
   */
  const title = computed(() => {
    return appStore.getInAppPopupTitle;
  });

  /**
   * Url
   */
  const url = computed(() => {
    return appStore.getInAppPopupUrl;
  });

  /**
   * Handle close button clicked
   */
  const handleCloseClick = async () => {
    appStore.hideInAppPopup();
  };

  /**
   * Handle update show
   */
  const handleUpdateShow = (show: boolean) => {
    if (!show) {
      appStore.hideInAppPopup();
    }
  };
</script>
