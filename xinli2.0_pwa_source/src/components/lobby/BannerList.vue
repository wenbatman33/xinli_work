<template lang="pug">
//- 廣告
.relative.w-full.h-40.flex.flex-col.items-center.justify-center
  //- 廣告讀取中
  Loading.z-10(v-if='bannerListLoading', type='spinner', size='16px')

  //- 廣告輪播
  Swiper.z-10.w-full.h-full.p-4(v-else, :modules='[EffectBanner, Autoplay]', :autoplay='{ delay: 3500, disableOnInteraction: false }', :loop='true', @slideChange='handleSlideChange')
    SwiperSlide.relative.h-full.w-full(v-for='item in bannerList')
      BannerListItem(:item='item')

  //- 當前輪播位置
  .absolute.z-20.right-10.bottom-8.flex.flex-row.flex-nowrap.justify-center.items-center.overflow-x-auto
...
<script setup lang="ts">
  ...
  import EffectBanner from '@/components/effect/EffectBanner';
  import BannerListItem from '@/components/lobby/BannerListItem.vue';
