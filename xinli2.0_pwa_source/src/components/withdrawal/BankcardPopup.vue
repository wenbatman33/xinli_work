...
Popup.min-h-full.bg-white(:show='show', :safeAreaInsetBottom='true', teleport='body', position='bottom', @update:show='handleUpdateShow')
  ...
  .grid.grid-rows-content.justify-stretch
    //- 導覽欄
    BasicNav(:title='t("withdrawal_chooseBankcard")')
      //- 左側
      template(#left)
        //- 關閉
    ...
    //- 銀行卡列表
    BankcardList(@clickItem='handleItemClick')
</template>
...
<script setup lang="ts">
  ...
  import BankcardList from '@/components/withdrawal-management/BankcardList.vue';
   ...
   * Handle item clicked
   */
  const handleItemClick = (item?: BankcardItem) => {
    emit('clickItem', item);
  };
</script>
