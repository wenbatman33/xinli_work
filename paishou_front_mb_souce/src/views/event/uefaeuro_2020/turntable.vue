<template lang="pug">
//- 歐洲杯轉盤
.w-full.min-h-full.bg-turntable.flex.flex-col.items-stretch
    //- 2021
    ...
    .w-full.flex.flex-col.items-center.pb-12
        //- 導覽欄
        navBar.self-stretch(title='转转好运来')

        //- 轉盤
        .self-stretch.relative.overflow-hidden.mt-12
            //- 底圖
            ...
            //- 按鈕
            van-button.absolute.z-30.position-roulette-button.p-0.rounded-full.bg-roulette-button.border-transparent(:loading='state.showLoading', @click='clickRoulette')
                img.w-full.h-full.object-contain(src='/publicAssets/img/event/uefaeuro_2020/icon-roulette-button@3x.png')
                template(#loading)
                    van-loading(type='spinner', color='#000000')

        //- 點數
        .self-stretch.flex.flex-row.justify-center.items-center.bg-point.rounded.mt-24.mx-6.p-4
            //- 標題
        ...
        //- 機會
        .text-sm.text-white.font-bold.opacity-50.mt-4 30点，1次机会

        //- 向下按鈕
        van-button.w-6.h-6.p-0.mt-5.rounded-full.bg-transparent.border-transparent(@click='clickDown')
            img.w-full.h-full(src='/publicAssets/img/event/uefaeuro_2020/icon-arrow-down-white@3x.png')

        //- 說明
        .text-sm.text-desc.text-center.bg-white.rounded.p-4.mx-6.mt-5
            span 累积30点获得一次转盘机会,华为Mate 40大奖等你拿
            br
            span 转盘一次扣除30点

        //- 獎項標題
        .text-base.text-white.font-bold.mt-12(ref='giftListRef') 转盘号码对应奖品

        //- 獎項列表
        .self-stretch.mx-6.mt-3
            //- 獎項
            .grid.grid-cols-gift.gap-2.items-center.bg-gift.rounded.p-4.mt-3(v-for='item in giftList')
                //- 編號
                .text-base.text-white.font-bold {{ item.id }}

                //- 圖示
                img.w-12.h-12.object-contain(:src='item.icon')

                //- 名稱
                .text-base.text-white.font-bold {{ item.name }}

        //- 向上按鈕
        van-button.w-8.h-8.p-0.mt-20.rounded-full.bg-transparent.border-transparent(@click='clickTop')
            img.w-full.h-full(src='/publicAssets/img/event/uefaeuro_2020/icon-arrow-up@3x.png')

        //- 版權聲明
        .text-sm.text-white.mt-28 Copyright © paishou 2021
    ...
    //- 獎勵彈窗
    bonus-popup(v-model:show='state.showBonus', :gift-icon='state.giftIcon', :gift-name='state.giftName', :gift-nums='state.giftNums', :gift-explain='state.giftExplain')
</template>
<script>
...
import bonusPopup from '@/views/event/uefaeuro_2020/bonusPopup.vue';
...
export default {
	...
	setup() {
		...
		// 點擊向下按鈕
		const clickDown = () => {
			giftListRef.value.scrollIntoView();
		};
