<template lang="pug">
.event-page(class='lg:my-8 lg:mx-auto lg:rounded')
	.back-home.text-white
		router-link.text-white(to='/')
			van-icon(name="cross")
	...
	.text-center.text-white.my-2.text-lg.font-bold
		span.cursor-pointer(@click='state.page=1', :class='{"text-psOrange": state.page===1}') 任务拿奖励 
		span.mx-2 |
		span.cursor-pointer(@click='state.page=2', :class='{"text-psOrange": state.page===2}') 转盘抽大奖 
		span.mx-2 |
		span.cursor-pointer(@click='state.page=3', :class='{"text-psOrange": state.page===3}') 蕉币买机会 
	.px-4
