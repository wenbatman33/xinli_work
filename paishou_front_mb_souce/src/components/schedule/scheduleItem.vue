...
.relative.my-2.mx-4
	...
	//- 物件內容
	.grid.grid-cols-content.items-center.w-full.bg-white.rounded-lg.shadow-content.overflow-hidden
		//- 賽事資訊
...
//- 賽程彈窗
schedule-popup(v-model:show='state.isShowPopup', :data='item', :path='path')
</template>
<script>
...
import { matomo_schedule_appointment, matomo_schedule_check_game } from '@/matomoTrackEvent.js';
...
export default {
	...
	setup(props, { emit }) {
		...
		// Matomo track schedule page
		const schedulePage = computed(() => {
			if (props.path === '/schedule/appointment') {
				return '我的預約';
			} else if (props.path === '/schedule/wonderful') {
				return '精彩賽事';
			} else if (/^\/schedule\/collaboration\//.test(props.path)) {
				const collaborationName = props.path.replace(/^\/schedule\/collaboration\//, '');
				return `賽程-${collaborationName}`;
		...
		const clickItem = () => {
			...
			// Matomo track
			matomo_schedule_check_game(schedulePage.value, props?.item?.eid);
		};
		...
		const clickAppointment = () => {
			...
			// Matomo track
			matomo_schedule_appointment(schedulePage.value, props?.item?.eid);
		};
