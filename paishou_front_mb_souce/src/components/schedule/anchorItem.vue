	...
	setup(props) {
		...
		const clickEnter = () => {
			...
			if (props.path === '/search') {
				matomo_search_watch(name.value, uid.value);
			} else if (props.path === '/schedule/appointment') {
				matomo_schedule_watch('我的預約', name.value, uid.value);
			} else if (props.path === '/schedule/wonderful') {
				matomo_schedule_watch('精彩賽事', name.value, uid.value);
			} else if (/^\/schedule\/collaboration\//.test(props.path)) {
				const collaborationName = props.path.replace(/^\/schedule\/collaboration\//, '');
				matomo_schedule_watch(`賽程-${collaborationName}`, name.value, uid.value);
			} else if (/^\/schedule\/class\//.test(props.path)) {
				const className = props.path.replace(/^\/schedule\/class\//, '');
				matomo_schedule_watch(`賽程-${className}`, name.value, uid.value);
			}
		};
		return {
			live,
