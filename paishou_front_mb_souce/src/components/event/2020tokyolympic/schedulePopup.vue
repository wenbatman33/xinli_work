...
van-popup(:show='show', position='bottom', round, teleport='.popupTeleport', @click-overlay='clickOverlay')
	//- 彈窗內容
	.flex.flex-col.items-stretch.pb-8.bg-popup
		//- 關閉按鈕
		van-button.w-full.h-auto.p-4.bg-transparent.border-transparent(@click='clickClose')
			img.w-4.h-4(src='/publicAssets/img/event/2020tokyolympic/icon-arrow-down@3x.png')
		...
		.grid.grid-cols-5.gap-2.mt-4.px-6
			...
			.flex.items-center.justify-center
				img.w-10.h-10.object-contain(src='/publicAssets/img/event/2020tokyolympic/icon-vs@3x.png')
		...
		.flex.flex-col.items-stretch.mt-2.px-6(v-else)
			...
			//- 列表
			anchor-list.mt-4(:list='combineAnchorList')
</template>
<script>
import { computed } from 'vue';
import { isLive, isUpcoming } from '@/utils';
import anchorList from '@/components/event/2020tokyolympic/anchorList.vue';
