<template lang="pug">
.mx-4.mt-4(v-if='state.slideList')
	.class-live-banner
...
import { matomo_classLive_click_slide } from '@/matomoTrackEvent.js';
...
export default {
	...
	setup(props) {
		...
		const className = computed(() => {
			...
			const classElement = classList?.find((element) => {
				return element.id === props.classId;
			});
		...
		const clickBanner = (item) => {
			...
			// Matomo track
			matomo_classLive_click_slide(className.value, item.slide_name, item.slide_id);
		};
